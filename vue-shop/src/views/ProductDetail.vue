...
/*
   File: ProductList.vue
    Content: 상품상세화면,/ detail눌렀을때 productDetail.vue가 보이게해주는 작업중
    Created: 손이영.
    Date: 2025.07.11
  */
export default {
  ...
  methods: {
    ...
    // 매개값으로 가격이 들어오면 format으로 반환해서 보여주게해
    getCurrencyFormat(price) {
      ...
      console.log(krwFormatter.format(price)); // → "₩1,234,567"
      return krwFormatter.format(price);
    },
    // 상세정보를 불러오는 함수
    async getProductDetail() {
      let product = await this.$api("/api/productDetail", {
        param: [this.productId], //node/app.js(83줄) 에서 param으로 값을 넘기기로 해서 param: 배열의 형태로 값을 넘김
      }); // 넘어가야하는 productId파라미터를 param에 담음
      // console.log(product[0]);
      this.productDetail = product[0]; // data안에 productDetail안에다가 값을 전달
  ...
  mounted() {
    ...
    this.productId = this.$route.query.product_id; // img안에 있는 product_id를 말함 // data에 있는 productId에 값이 넘어감
