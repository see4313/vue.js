...
/*
   File: ImageInsert.vue
    Content: 상품등록,/ image_insert눌렀을때 ImageInsert.vue가 보이게해주는 작업중
    Created: 손이영.
    Date: 2025.07.11
  */
export default {
  data() {
    return {
      productId: 0,
      productDetail: {}, //{}: 객체
      productImage: [], //[]: 배열 (list 형식이라 배열사용)
    };
  },
  created() {
    this.productId = this.$route.query.product_id;
    this.getProductDetail(); //상품상세정보가져오기
    this.getProductImage(); //이미지 전체목록
  },
  methods: {
    async deleteImage(id) {
      let result = await this.$api("/api/imageDelete", { param: [id] });
      console.log(result);
      alert("삭제완료!");
    },
    ...
    async uploadFile(files, type) {
      ...
      let data = await this.$base64(files[0]); // (files[0])는 첫번째 파일을 전달
